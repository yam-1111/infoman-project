...
class EntityParser:
    ...
    def jsonParse(self):
        ...
        json_to_sql = {v: k for k, v in PERSONALINFO_PARSER_DICT.items()}
    ...
    def sqlParse(self):
        ...
        sql_dict = {}
        for key, value in self.__dict__.items():
            if key in PERSONALINFO_PARSER_DICT:
                sql_key = PERSONALINFO_PARSER_DICT.get(key)
                sql_dict[sql_key] = value
        return sql_dict
    

    def sqlTupleToJson(self, sql_column_list: list, sql_tuple: tuple):
        """
        ...
        for column, value in zip(sql_column_list, sql_tuple):
            if column in json_to_sql:
                if column == 'Date_Of_Birth':  
                    json_dict[json_to_sql[column]] = str(value) if value is not None else ''
                else:
